...
module.exports = {
  ...
  /**
   * Use the built in body parser, if set
   * to FALSE will use raw response
   */
  useBodyParser: true,
  ...
  sendErrorResponse: function(err) {
    ...
    this.res.format({
      ...
      'text/html': function() {
        ...
        } else if ('object' === typeof config[err.statusCode]) {
  ...
  ai: {
    ...
    /**
     * Called before image is sent
     * @param res
     */
    beforeSend(res, stream) {},

    /**
     * Set maxAge to set cache control header
     */
    maxAge: null,
