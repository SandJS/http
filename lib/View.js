...
class View {
  ...
  static printJS(files) {
    ...
    if ('development' === sand.env) {
      _.each(files, function (name) {
        jsStr += '<script type="text/javascript" src="/js/' + name + '.js"></script>';
      });
    } else {
      if (files && files.length > 0) {
        jsStr += `<script type="text/javascript" src="${sand.static.minifiedJSURL(files)}"></script>`
      }
    }
  ...
  static printCSS(files) {
    ...
    if ('development' === sand.env) {
      ...
      if (files && files.length > 0) {
        cssStr += `<link type="text/css" rel="stylesheet" href="${sand.static.minifiedCSSURL(files)}">`
      }
    }
