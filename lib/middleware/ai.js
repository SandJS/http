...
module.exports = function(http) {
  ...
  return function(req, res, next) {
    ...
    function adapt(callback) {
      ...
      img.size(function(err, nativeSize) {
        ...
        if (!imageWidth && !imageHeight) {
          ...
          let resolvedWidth = imageWidth || Math.round(imageHeight * (nativeSize.width / nativeSize.height));
          let resolvedHeight = imageHeight || Math.round(imageWidth * (nativeSize.height / nativeSize.width));

          allowResize = resolvedWidth < nativeSize.width && resolvedHeight < nativeSize.height;
        } else {
          // Two sizes provided.
     ...
     */
    function getKey() {
      let paramString = width + 'x' + height + '-' + path + (is2x ? '-@2x' : '') + (bg ? '_' + bg : '');
