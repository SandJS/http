...
class Http extends SandGrain {
  ...
  init(config, done) {
    ...
    // Lets register the domain, can be access using process.domain
...
//
//      if (typeof sand.session !== 'undefined') {
...
//    if (_.isFunction(this.config.beforeMiddleware)) {
//      this.config.beforeMiddleware(this.express);
//    }
...
//    if (_.isFunction(this.config.afterMiddleware)) {
//      this.config.afterMiddleware(this.express);
//    }
//
//    // Register 404
...
//
//    if (this.config.spdy) {
//      this.server = spdy.createServer(this.config.spdy, this.express);
//      this.server.listen(this.config.port, listening.bind(this));
//
//    } else {
//      this.server = this.express.listen(this.config.port, listening.bind(this));
//    }
//
//
...
//
//    function listening() {
//      this.log('Listening on ' + this.config.port + ( this.config.spdy ? ' with SPDY enabled'.red : '' ));
//      done();
//    }
//
//    return this;
...
//      this.logPolicy('WARN: Missing policy file. Skipping loading of policies...');
//      return;
//    }
//
...
//      self.logPolicy('Mapping: ' + pattern);
//
//      if (_.isArray(policy)) {
//        _.each(policy, function(p) {
//          registerPolicy(p, pattern);
//        });
//      } else {
//        registerPolicy(policy, pattern);
//      }
//    });
//
//    function registerPolicy(policy, pattern) {
//      self.express.use(pattern, function(req, res, next) {
//        var name = policy.name || pattern;
//        self.logPolicy('Running: ' + name);
//        if (!req.skipToAction) {
