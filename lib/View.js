...
class View {
   ...
   */
  static mixinRender(res, http) {
    let config = http.config;
    res._render = res.render;

    let logView = http.log.as('http:view');
      ...
      res._render(view, data, function (err, result) {
        if (err) {
          ...
          if (layout === false) {
            ...
            data.body = result;
            res._render(View.getPaths(layout, true, config).full, data, function (err, result) {
              if (err) {
                logView.error(err.message);
              }
